        ...
        if user:
            ...
            if check_password(password, user['password']):
                ...
                if otp_created:
                    ...
                    # OTP email failed — log user in directly as fallback
                    print(f"⚠️ OTP email failed, logging in directly as fallback")
                    ...
                    if user['role'] == 'customer':
                        ...
                        cursor2.execute("SELECT * FROM customers WHERE user_id = %s", (user['id'],))
                        customer = cursor2.fetchone()
                        if customer:
                            session['customer_id'] = customer['id']
                            session['name'] = f"{customer['first_name']} {customer['last_name']}"
                    elif user['role'] == 'seller':
                        cursor2 = get_db_connection().cursor()
                        cursor2.execute("SELECT * FROM sellers WHERE user_id = %s", (user['id'],))
                        seller = cursor2.fetchone()
                        if seller:
                            session['seller_id'] = seller['id']
                            session['name'] = seller['business_name']
                    
                    flash('Login successful! (Email verification unavailable)', 'success')
                    if user['role'] == 'admin':
